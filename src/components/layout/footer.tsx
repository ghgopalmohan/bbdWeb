      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 items-center">
          <div className="flex justify-center md:justify-start">
            <Link href="#home" className="font-headline text-5xl font-bold text-white" data-cursor-type="pointer">
              GM
            </Link>
          </div>
          ...
          <nav className="flex flex-wrap justify-center md:justify-end gap-5 md:gap-8">
            {footerNavItems.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className="text-base font-medium transition-colors hover:text-white text-gray-300"
